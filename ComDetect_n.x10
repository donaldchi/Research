...
public class ComDetect_n {
	...
	public static def one_pass(var id:Int, var startIndex:Int, var endIndex:Int, var args:Array[String], var ComDe:ComDetect_n, var indexs:Array[Int],var com:Comcal_opt,var total_weight:Double){
		...
		var i:Int;
		for( i = 0; i < ComDe.g.links.size; i++ ){
		finish {	
			async{

			//Console.OUT.println("Index = " + i);
			if(isR)
			{
				frontID = indexs(i);
			}
    ...
    public  static def  main(args: Array[String]) {
    	...
    	// Console.OUT.print("before::");
         ...
         //display(links_opt,node2com,comInfo_obj);
    	    ...
    	    /*
    	    var band:Int = comDe.g.links.size / threadNums;
    	    finish 
    	    {    
            	for ( threadId = 0; threadId < threadNums; threadId++ ){
                	...
                	{
                		if( id == (threadNums-1) )
                			comDe.one_pass( id, startIndex, endIndex, args, comDe, indexs, com, total_weight );
                		else 
                			comDe.one_pass( id, startIndex, comDe.g.links.size, args, comDe, indexs, com, total_weight );
                	}
            	}   
            } */  

    	    comDe.one_pass( 0,0, comDe.g.links.size, args, comDe, indexs, com, total_weight );
